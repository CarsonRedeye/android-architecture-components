...
class NetworkBoundResourceTest(private val useRealExecutors: Boolean) {
    ...
    fun basicFromNetwork() {
        ...
        verify(observer).onChanged(Result.success(fetchedDbValue))
    }
    ...
    fun failureFromNetwork() {
        ...
        verify(observer).onChanged(Result.error("error", null))
    ...
    fun dbSuccessWithoutNetwork() {
        ...
        dbData.value = dbFoo
        drain()
        verify(observer).onChanged(Result.success(dbFoo))
        assertThat(saved.get(), `is`(false))
        val dbFoo2 = Foo(2)
        dbData.value = dbFoo2
        drain()
        verify(observer).onChanged(Result.success(dbFoo2))
        verifyNoMoreInteractions(observer)
    }
    ...
    fun dbSuccessWithFetchFailure() {
        ...
        drain()
        verify(observer).onChanged(Result.loading(dbValue))

        apiResponseLiveData.value = ApiResponse.create(Response.error<Foo>(400, body))
        drain()
        assertThat(saved.get(), `is`(false))
        verify(observer).onChanged(Result.error("error", dbValue))

        val dbValue2 = Foo(2)
        dbData.value = dbValue2
        drain()
        verify(observer).onChanged(Result.error("error", dbValue2))
        verifyNoMoreInteractions(observer)
    }
    ...
    fun dbSuccessWithReFetchSuccess() {
        ...
        drain()
        assertThat(saved.get(), `is`(networkResult))
        verify(observer).onChanged(Result.success(dbValue2))
        verifyNoMoreInteractions(observer)
    }
